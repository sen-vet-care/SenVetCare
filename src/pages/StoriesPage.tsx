  ...
  useEffect(() => {
    async function fetchBlogs() {
      try {
        ...
        console.warn('Failed to fetch from blogs table:', err);
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    <section id="blog" className="py-32 max-w-[1280px] mx-auto px-6 relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 relative z-10">
        {loading && blogs === PLACEHOLDER_BLOGS ? (
          <div className="col-span-full py-20 text-center text-on-surface-variant font-inter">Loading insights...</div>
          ...
          filteredBlogs.map((blog, index) => {
            ...
            return (
              <article 
                key={blog.id} 
                className={`${isFeatured ? 'lg:col-span-2' : 'col-span-1'} bg-surface-container-low p-8 rounded-[2rem] shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-500 border border-outline-variant/30 group cursor-pointer flex flex-col`}
              >
